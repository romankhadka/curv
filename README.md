...
- **fancurved** is a LaunchDaemon that runs as root. Every 2 seconds it reads
  that file, averages the CPU-die keys (`Tp*`) and the GPU keys (`Tg*`), takes
  the hotter one, interpolates the curve, and writes each fan's target RPM
  through the SMC. Percent maps onto each fan's own minimum and maximum.
...
- At **100 °C** or above it forces 100% regardless of the curve. macOS thermal
  throttling stays in force at all times; Curv only sets fan targets.
